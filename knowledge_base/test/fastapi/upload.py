...
ALLOWED_TYPES = ["application/pdf", "text/markdown"]
...
@app.post("/upload", summary="文件上传", response_model=UploadResponse)
async def upload(
    ...
    try:
        ...
        if not validate_file_type(file.content_type, ALLOWED_TYPES):
            raise HTTPException(
                status_code=400,
                detail=f"文件类型错误，请上传 {','.join(ALLOWED_TYPES)} 类型的文件"
            )
        ...
        # 4. 组装文件数据(文件 UploadResponseData)
