...
# 4. 定义异步方法:做包子
async def zuobaozi(session_id: str):
    ...
    for i in range(10):
        ...
        msg = f"这是会话{session_id}的第{i + 1}个包子"
        # 模拟耗时
...
async def submit_task(session_id: str, background_tasks: BackgroundTasks):
    ...
    background_tasks.add_task(zuobaozi, session_id)
    return {"message": f"会话{session_id}的包子订单提交成功"}
